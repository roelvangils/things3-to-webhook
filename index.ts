...
const getThingsData = (): any[] => {
    try {
        const output = Bun.spawnSync(
            ['things-cli', '--json', '-t', 'Sunsama', 'logbook'],
            {
                stdout: 'pipe',
            },
        );
...
const prepareDataForSubmission = (
    thingsData: any[],
    syncedUUIDs: Set<string>,
): { data: any[]; newUUIDs: Set<string> } => {
...
const main = async () => {
    ...
    const { data: filteredData, newUUIDs } = prepareDataForSubmission(
        thingsData,
        syncedUUIDs,
    );
    ...
    if (filteredData.length > 0) {
        ...
        const updatedSyncedUUIDs = new Set([
            ...Array.from(syncedUUIDs),
            ...Array.from(newUUIDs),
        ]);
        writeSyncedUUIDs(updatedSyncedUUIDs);
        console.log(`Synced ${filteredData.length} new todos`);
        // execSync(`speak "${filteredData.length} new todos added to Sunsama" &`);
    } else {
        console.log('No new todos to sync');
        // execSync('speak "No new todos to sync" &' );
    }
};
